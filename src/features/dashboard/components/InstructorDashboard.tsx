...
export default function InstructorDashboard() {
  ...
  useEffect(() => {
    async function getInitialData() {
      ...
      // Role guard â€” instructor or admin only
        ...
        <div className="flex items-center justify-between px-8 h-20 w-full max-w-screen-2xl mx-auto">
          <div className="flex items-center gap-8">
            <Link href="/" className="flex items-center gap-2 active:scale-95 transition-transform outline-none">
              <img
                alt="Ø¬Ø§Ù…Ø¹Ø© ÙØ§ÙŠØ±ÙŠØ± Ø§Ù„Ø³Ø¹ÙˆØ¯ÙŠØ©"
              ...
              />
              <span className="text-2xl font-black text-primary tracking-tighter font-headline">Ø¬Ø§Ù…Ø¹Ø© ÙØ§ÙŠØ±ÙŠØ± Ø§Ù„Ø³Ø¹ÙˆØ¯ÙŠØ©</span>
            </Link>
              ...
              <div className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-accent via-violet-600 to-primary p-8 mb-4">
                ...
                <div className="relative z-10 flex flex-col xl:flex-row xl:items-center justify-between gap-6">
                  <div>
                    ...
                    <p className="text-white/70 text-sm">Your Ø¬Ø§Ù…Ø¹Ø© ÙØ§ÙŠØ±ÙŠØ± Ø§Ù„Ø³Ø¹ÙˆØ¯ÙŠØ© performance is updated in real-time.</p>
                  </div>
                  <div className="flex flex-wrap items-center gap-3 flex-shrink-0">
                    <button
                      onClick={() => {
                        const text = `My Ø¬Ø§Ù…Ø¹Ø© ÙØ§ÙŠØ±ÙŠØ± Ø§Ù„Ø³Ø¹ÙˆØ¯ÙŠØ© instructor stats â€” check out my courses!`;
                        if (navigator.share) {
                          navigator.share({ title: 'Ø¬Ø§Ù…Ø¹Ø© ÙØ§ÙŠØ±ÙŠØ± Ø§Ù„Ø³Ø¹ÙˆØ¯ÙŠØ©', text, url: window.location.origin });
                        } else {
                          navigator.clipboard.writeText(`${text} ${window.location.origin}`);
                          alert('ØªÙ… Ù†Ø³Ø® Ø§Ù„Ø±Ø§Ø¨Ø·! Stats link copied to clipboard.');
                        }
                      }}
                            ...
                            <div key={m.label + i} className="flex flex-col items-center gap-3 w-full h-full justify-end">
                              <div className="relative w-full group" style={{ height: `${Math.max(pct, 2)}%` }}>
                                ...
                                <div className="absolute -top-7 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity bg-on-surface text-white text-[10px] font-bold px-2 py-1 rounded whitespace-nowrap z-10">
                                  ${m.revenue.toLocaleString()} Â· {m.enrollments} enr.
                                </div>
                              </div>
